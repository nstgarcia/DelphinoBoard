...
//Low-level functions for GPIO configuration (CPU1 only)
	...
	void InitGpio()
	{
		...
		for (regOffset = 0; regOffset < sizeof(GpioCtrlRegs)/2; regOffset++)
		{
			...
			if (regOffset % (0x40/2) != (0x0C/2))
				gpioBaseAddr[regOffset] = 0x00000000; //no escribe en el GPyPUD
		}

		gpioBaseAddr = (Uint32 *)&GpioDataRegs;
		//las direcciones de los regs aumentan de 2 en 2, por eso es la mitad
	...
	void GPIO_SetupPinMux(Uint16 pin, Uint16 cpu, Uint16 peripheral)
	{
		...
		pin16 = pin % 16;
		pin8 = pin % 8;
		//(pin/32), obtiene el grupo al que corresponde el pin
		//GPY_CTRL_OFFSET: (0x40/2)->32 decimal
		gpioBaseAddr = (Uint32 *)&GpioCtrlRegs + (pin/32)*GPY_CTRL_OFFSET;//pin/32, saca el grupo
		//i.e 32/32 =1er grupo; cada 40h hay un y_ctrl_reg
		...
		//GPYMUX: GPIO Y Mux X Register  0X6
		//GPAYMUX1 GPIO Y Peripheral Group Mux (GPIO0 to 15)
		//GPAYMUX2 GPIO Y Peripheral Group Mux (GPIO16 to 31)
		//se divide entre 16, porque se debe sumar uno, es decir, ir al registro del mux2, si
		//es mayor a 15

		mux = gpioBaseAddr + GPYMUX + pin32/16;
		gmux = gpioBaseAddr + GPYGMUX + pin32/16; //16
	...
	//GPIO_PULLUP	Enable pull-up (1 << 0)
	//GPIO_INVERT	Enable input polarity inversion (1 << 1)
	//GPIO_SYNC		Synchronize the input latch to PLLSYSCLK (default -- you don't need to specify this)
			//(0x0 << 4)
	//GPIO_QUAL3	Use 3-sample qualification (0x1 << 4)
	//GPIO_QUAL6	Use 6-sample qualification (0x2 << 4)
	//GPIO_ASYNC	Do not use synchronization or qualification (0x3 << 4)
	//(Note: only one of SYNC, QUAL3, QUAL6, or ASYNC is allowed)
	//
	//For output pins, the valid flags are:
	//GPIO_OPENDRAIN	Output in open drain mode (1 << 2)
	//GPIO_PULLUP		If open drain enabled, also enable the pull-up (1 << 0)
	...
	void GPIO_SetupPinOptions(Uint16 pin, Uint16 output, Uint16 flags)
	{
		...
		pin32 = pin % 32; //filtra el pin
		pin16 = pin % 16;
		pinMask = 1UL << pin32; //i.e. si es 33, 33%32=1, se usará el pin 1 de algún grupo
		gpioBaseAddr = (Uint32 *)&GpioCtrlRegs + (pin/32)*GPY_CTRL_OFFSET ;//pin/32, saca el grupo
		//i.e 32/32 =1er grupo; cada 40h hay un y_ctrl_reg
		...
		*dir &= ~pinMask; //as input by default
		//GPyDIR ->0: input, 1: output
		if (output == 1) //si es 1 , es output
		{
			//Output, with optional open drain mode and pull-up
			*dir |= pinMask;

			//Enable open drain if necessary
			if (flags & GPIO_OPENDRAIN)//mask:
				*odr |= pinMask;
			else
				*odr &= ~pinMask;
...
Uint16 GPIO_ReadPin(Uint16 pin)
{
	...
	pinVal = (gpioDataReg[GPYDAT] >> (pin % 32)) & 0x1; //lo recorre 32 lugares
