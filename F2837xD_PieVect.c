...
//---------------------------------------------------------------------------
/*La siguiente estructura inicializa la estructura PIE_VECT_TABLE
 * o sea, en el archivo PIEVect.h se define la estructura abstracta general de la tabla
 * de vectores. Después hay una variable externa "PieVectTable", que será el destino,
 * o la variable a usar para manipulacion.
 * Ahora, aqui hay una estructura "const PIE_VECT_TABLE" llamada PieVectTableInit. Ésta
 * inicializa la tabla de vectores. Es decir, guardará las rutinas ISR reales que manejan
 * a las interrupciones. Dichas rutinas están definidas en el archivo "DefaultISR.c".
 * */
...
/*El PieVectTableInit es la estructura con las rutinas ISR guardadas.
 * El contenido que está en la direccion de esta estructura se guardaré en el
 * apuntador Source. La variable PieVectTable es una variable global externa definida en
 * el globalvariables. Luego el contenido de la dirección de esa estructura se guarda en
 * el apuntador Dest.
 * */
...
// initialized by Boot ROM with boot variables)
    ...
    Source  =  Source  +  3;  //se aumenta la direccion más 3
    Dest  =  Dest  +  3;  

    EALLOW;  
    for(i  =  0;  i  <  221;  i++)  
    {  
    *Dest++  =  *Source++;  //se accede al contenido y se aumenta la direccion
    }  
    EDIS;  
